...
(This turns out to be more convenient than writing $P(x, y)$ using symbols such as $P_{ij}$, as well as aligning better with the infinite state case.)
...
A Markov chain $(X_t)_{t \in \mathbb Z_+}$ on $S$ with Markov matrix $P$ is a
...
It is now clear that $P$ defines the joint distribution when paired 
with any initial condition $\psi$.
...
What is the probability that $X_{s+t} = i$ given both the history $(X_r)_{r \leq s}$ and current information $X_s = i$?

If $t$ is small, then this is close to the
probability that there are zero switches between $s$ and $t$.

But to calculate this probability, it would be helpful to know how long the
state has been at i.
...
As a result, the history prior to $X_s$ is useful for predicting $X_{t+s}$,
even when we know $X_s$.

Thus, the Markov property fails.

In fact the Markov property {ref}`will only hold <exp_unique>` if $W$ is drawn exponentially instead.

[XX why isn't this link working??]

This helps to make clear how much structure the Markov property imposes.



### From Semigroup to Joint Distribution

We saw {ref}`above <jdfin>` that, in the discrete time case, the joint
distribution of $(X_t)$ is determined by its Markov kernel plus an
initial condition (distribution for $X_0$).

The same is true in the continuous time case if we replace Markov kernel
with transition semigroup.

The proof is very similar to the discrete time case and can be found in
Corollary 6.4 of {cite}`le2016brownian`.
...
Let's look at a new example of a continuous time Markov process.
...
IID draw from the geometric distribution
...
The Poisson process is a jump process on $S = \mathbb Z_+$.
...
```{code-cell} ipython3
    ...
    def plot_distribution_dynamics(ax, ψ_0, steps=200, step_size=0.1):
        ...
        colors = cm.plasma_r(np.linspace(0, 1, steps))

        for i in range(steps):
            if i == 0:
                ax.plot(states, ψ, 'o', lw=0.5, color=colors[i], label='$t=0$')
            else:
                ax.plot(states, ψ, 'o', lw=0.5, color=colors[i])
            ψ = P_t(ψ, t=step_size)

        ax.plot(states, ψ, 'ko-', label=f'$t={steps * step_size}$')    
        ax.legend(loc='upper center', ncol=2)
        ax.set_xlabel('inventory levels')
        ax.set_ylim((0, 0.32))
...
Construct two different random variables that have this distribution.
